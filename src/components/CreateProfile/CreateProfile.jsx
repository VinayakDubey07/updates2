...
const CreateProfile = () => {
  ...
  const handleSubmit = async () => {
    if (
      studentDetails?.Class.length === 0 ||
      studentDetails?.board.length === 0 ||
      studentDetails?.dob.length === 0 ||
      studentDetails?.gender.length === 0 ||
      studentDetails?.interests.length === 0 ||
      studentDetails?.mobile.length === 0
    ) {
      ...
      try {
        const response = await fetch(
          "https://exquisite-backend-test.herokuapp.com/api/student_profiles//addprofile",
          {
            method: "POST",
            headers: {
